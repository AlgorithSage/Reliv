...
files.forEach(file => {
  ...
  >
    ${text}
        ...
        // Safer regex: Match a MaterialButton block that contains the text, 
        // ensuring no OTHER <MaterialButton tag is inside the match.
        const regex = new RegExp('<MaterialButton((?!<MaterialButton)[\\s\\S])*?' + text.replace(/[!]/g, '\\$1') + '[\\s\\S]*?<\\/MaterialButton>');
